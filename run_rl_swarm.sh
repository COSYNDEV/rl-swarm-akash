...
if [ "$NUM_GPUS" -gt 1 ] && [ -z "$CPU_ONLY" ]; then
    echo_green ">> Detected $NUM_GPUS GPUs. Using torchrun for distributed training..."
    if [ -n "$ORG_ID" ]; then
        torchrun --nproc_per_node=$NUM_GPUS -m hivemind_exp.gsm8k.train_single_gpu \
